      ...
      <nav className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <Link to="/" className="flex items-center gap-1">
            <div className="h-5 w-auto sm:h-6">
              <Logo />
            </div>
            <span className="hidden text-[18px] font-medium tracking-tight sm:inline">
              YouTube
            </span>
          </Link>
        </div>
        ...
        {/* Right: mic, create, notifications, profile, theme */}
        <div className="flex items-center gap-1 sm:gap-2">
          <button
            type="button"
            className="hidden h-9 w-9 items-center justify-center rounded-full bg-zinc-100 text-zinc-800 hover:bg-zinc-200 dark:bg-zinc-800 dark:text-zinc-100 dark:hover:bg-zinc-700 sm:inline-flex"
        ...
        <button className="group peer ml-1 flex w-7 shrink-0 flex-wrap gap-y-1.5 sm:hidden">
          <span className="block h-[2px] w-full bg-zinc-900 group-hover:bg-[#08e6f5] dark:bg-white" />
          <span className="block h-[2px] w-2/3 bg-zinc-900 group-hover:bg-[#08e6f5] dark:bg-white" />
          <span className="block h-[2px] w-full bg-zinc-900 group-hover:bg-[#08e6f5] dark:bg-white" />
        </button>
        <div className="fixed inset-y-0 right-0 flex w-full max-w-xs shrink-0 translate-x-full flex-col border-l border-white bg-[#121212] text-white duration-200 hover:translate-x-0 peer-focus:translate-x-0 sm:static sm:ml-4 sm:w-auto sm:translate-x-0 sm:border-none">
          ...
          <ul className="my-4 flex w-full flex-wrap gap-2 px-4 sm:hidden">
            ...
            <li className="w-full">
              <Link to={user ? `/mychannel/${user.username}` : '/login'} className="flex w-full items-center justify-start gap-x-4 border border-white px-4 py-1.5 text-left hover:bg-[#08e6f5] hover:text-black focus:border-[#08e6f5] focus:bg-[#08e6f5] focus:text-black">
                <span className="inline-block w-full max-w-[20px] group-hover:mr-4 lg:mr-4">
                  <svg
                    style={{ width: "100%" }}
                    viewBox="0 0 22 16"
              ...
              <div className="flex w-full items-center gap-4 md:w-auto">
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-3 rounded-full px-2 py-1 text-left hover:bg-white/10"
                  >
                    <img
                      src={user?.avatar}
                  ...
                  {isProfileOpen && (
                    <div className="absolute right-0 z-20 mt-2 w-48 rounded-lg border border-zinc-700 bg-zinc-900/95 p-1 text-sm shadow-xl">
                      <button
                        type="button"
                        className="flex w-full items-center rounded px-3 py-2 text-left hover:bg-white/10"
                        onClick={() => {
                          navigate(`/mychannel/${user?.username}`);
                          setIsProfileOpen(false);
                        }}
                      >
                        My channel
